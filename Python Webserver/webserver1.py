...
class requestHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        if self.path == '/new':
            ...
            client_ip = self.client_address[0] # get IP address of client that sent the request
            tasklist.append(task + ", " + timestamp + " (" + client_ip + ")")
            if len(tasklist) > 10:
                del tasklist[0]
...
def main():
    PORT = 8000
    server_address = ('192.168.4.36', PORT) # use the IP address of the computer running the server
